...
router.get('/:id', async (req, res) => {
  try {
    const data = await prisma.vehiculo.findUnique({ where: { id: Number(req.params.id) } });
...
router.post('/', async (req, res) => {
  try {
    const data = await prisma.vehiculo.create({ data: req.body });
...
router.put('/:id', async (req, res) => {
  try {
    const data = await prisma.vehiculo.update({ where: { id: Number(req.params.id) }, data: req.body });
    res.json({ success: true, data });
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message });
  }
