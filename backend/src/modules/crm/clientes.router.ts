...
router.get('/count', async (req, res) => {
  ...
  const filters = where ? JSON.parse(where) : {};
  try {
    const count = await prisma.cliente.count({ where: filters });
...
router.get('/:id', async (req, res) => {
  try {
    const data = await prisma.cliente.findUnique({ where: { id: Number(req.params.id) } });
...
router.put('/:id', async (req, res) => {
  try {
    const data = await prisma.cliente.update({ where: { id: Number(req.params.id) }, data: req.body });
    res.json({ success: true, data });
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message });
  }
